...
To meet the needs of some industrial and commercial applications, as well as requirements in the areas of health and safety, an _expanded uncertainty_ stem:[ii(U)] is obtained by multiplying the combined standard uncertainty stem:[u_{"c"}] by a _coverage factor_ stem:[k]. The intended purpose of stem:[ii(U)] is to provide an interval about the result of a measurement that may be expected to encompass a large fraction of the distribution of values that could reasonably be attributed to the measurand. The choice of the factor stem:[k], which is usually in the range 2 to 3, is based on the coverage probability or level of confidence required of the interval (see <<cls_6>>).
