...
coefficient of stem:[w_i] and stem:[w_j], where stem:[ii(nu)(w_i, w_j) = ii(E)[(w_i - ii(mu)_i)(w_j - ii(mu)_j)\]] is the covariance of stem:[w_i] and stem:[w_j].


NOTE: stem:[ii(sigma)_z^2] and stem:[ii(sigma)_i^2] are, respectively, the central moments of order 2 (see <<scls_C-2-13>> and <<scls_C-2-22>>) of the probability distributions of stem:[z] and stem:[w_i]. A probability distribution may be completely characterized by its expectation, variance, and higher-order central moments.
...
Evaluations based on repeated observations are not necessarily superior to those obtained by other means. Consider stem:[s(bar(q))], the experimental standard deviation of the mean of stem:[n] independent observations stem:[q_k] of a normally distributed random variable stem:[q] [see <<eq_5>> in <<scls_4-2-3>>]. The quantity stem:[s(bar(q))] is a statistic (see <<scls_C-2-23>>) that estimates stem:[ii(sigma)(bar(q))], the standard deviation of the probability distribution of stem:[bar(q)], that is, the standard deviation of the distribution of the values of stem:[bar(q)] that would be obtained if the measurement were repeated an infinite number of times. The variance stem:[ii(sigma)^2[s(bar(q))\]] of stem:[s(bar(q))] is given, approximately, by
