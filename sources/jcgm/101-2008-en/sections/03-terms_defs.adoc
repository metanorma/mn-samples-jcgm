...
[[cls_3]]
== Terms and definitions (((Guide to the expression of uncertainty in measurement (GUM))))(((International vocabulary of basic and general terms in metrology (VIM))))
...
=== probability distribution (((probability)))(((probability distribution)))(((probability distribution,joint)))(((probability distribution,multivariate)))(((probability distribution,univariate)))
domain:[random variable]

function giving the probability that a ((random variable)) takes any given value or belongs to a given set of values
...
=== probability density function (((probability density function (PDF))))(((probability element)))

derivative, when it exists, of the ((distribution function))

[stem%unnumbered]
++++
g_X (xi) = "d"ii(G)_X (xi)//"d"xi
++++
...
[stem%unnumbered]
++++
g_X (xi) "d"xi = "Pr"(xi < ii(X) < xi + "d"xi).
++++
====
...
=== normal distribution (((Gaussian distribution)))(((Gaussian distribution,probability density function)))(((normal distribution)))

probability distribution of a continuous random variable stem:[ii(X)] having the probability density function

[stem%unnumbered]
++++
g_X (xi) = 1 /{ii(sigma) sqrt(2 pi)} exp[- 1/2 ({xi - ii(mu)}/ii(sigma))^2],
++++

for stem:[-infty < xi < +infty]


NOTE: stem:[ii(mu)] is the expectation and stem:[ii(sigma)] is the standard deviation of stem:[ii(X)].
...
=== stem:[t]-distribution (((gamma function)))(((t-distribution)))(((t-distribution,probability density function)))
...
[stem%unnumbered]
++++
g_x(xi) = (Gamma((nu + 1)//2)) / {sqrt(pi nu)ii(Gamma) (nu//2)} (1 + xi^2/nu)^{-(nu+1)//2},
++++


for stem:[-oo < xi < +oo], with parameter stem:[ii(nu)], a positive integer, the ((degrees of freedom)) of the distribution, where

[stem%unnumbered]
++++
ii(Gamma)(z) = int_0^{oo} t^{z-1} e^{-t} " d"t, " " " " z > 0
++++
...
[NOTE]
====
The expectation of the random variable stem:[ii(Z) = ii(F)(ii(X))], for a given function stem:[ii(F)(ii(X))], is

[stem%unnumbered]
++++
ii(E)(ii(Z)) = ii(E)(ii(F)(ii(X))) = int_{-oo}^{oo} ii(F)(xi) g_X (xi) " d"xi.
++++
====
...
=== moment of order stem:[r] (((moment)))

expectation of the __r__th power of a random variable, namely

[stem%unnumbered]
++++
ii(E)(ii(X)^r) = int_{-oo}^{oo} xi^r g_X (xi) " d"xi
++++

NOTE: The central moment of order stem:[r] is the expectation of the random variable stem:[ii(Z) = [ii(X) - ii(E)(ii(X))\]^r].
...
property of a pair of random variables, which, for two continuous random variables stem:[ii(X)_1] and stem:[ii(X)_2] characterized by a joint (multivariate) PDF stem:[g_{bb(X)} (bb(xi))], where stem:[ii(X) = (ii(X)_1, ii(X)_2)^{sf(T)}] and stem:[xi = (xi_1, xi_2)^{sf(T)}], is given by

[stem%unnumbered]
++++
"Cov"(ii(X)_1,ii(X)_2) = int_{-oo}^{oo} int_{-oo}^{oo} [xi_1 - ii(E)(ii(X)_1)][xi_2 - ii(E)(ii(X)_2)] g_{bb(X)} (xi) " d"xi_1 " d"xi_2
++++

NOTE: Not all pairs of random variables have a covariance.


=== uncertainty matrix (((covariance)))(((covariance matrix)))(((mutual uncertainty)))(((uncertainty matrix)))(((variance-covariance matrix)))
...
where stem:[u(x_i, x_i) = u^2(x_i)] is the variance (squared ((standard uncertainty))) associated with stem:[x_i] and stem:[u(x_i, x_j)] is the covariance associated with stem:[x_i] and stem:[x_j]. stem:[u(x_i, x_j) = 0] if elements stem:[ii(X)_i] and stem:[ii(X)_j] of stem:[bb(X)] are uncorrelated.
...
=== coverage interval (((Bayesian interval)))(((coverage interval)))(((credible interval)))
...
=== coverage probability (((coverage probability)))(((level of confidence)))
...
=== length of a coverage interval (((coverage interval,length of)))

largest value minus smallest value in a coverage interval


=== probabilistically symmetric coverage interval (((coverage interval,probabilistically symmetric)))
...
=== shortest coverage interval (((coverage interval,shortest)))
...
=== propagation of distributions (((propagation of distributions)))
...
=== GUM uncertainty framework (((GUM uncertainty framework (GUF))))
...
=== Monte Carlo method (((Monte Carlo method (MCM))))
