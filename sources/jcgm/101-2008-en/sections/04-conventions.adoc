...
where stem:[ii(Y)] is a scalar output quantity and stem:[bb(X)] represents the stem:[ii(N)] input quantities stem:[(ii(X)_1,...,ii(X)_N )^{sf(T)}]. Each stem:[ii(X)_i] is regarded as a random variable with possible values stem:[xi_i] and expectation stem:[x_i]. stem:[ii(Y)] is a random variable with possible values stem:[eta] and expectation stem:[y].
