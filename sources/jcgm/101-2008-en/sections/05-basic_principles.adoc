...
Since stem:[ii(G)_Y (eta)] encodes all the information known about stem:[ii(Y)], any property of stem:[ii(Y)] such as expectation, variance and coverage intervals can be approximated using stem:[bb(G)]. The quality of these calculated results improves as the number of times the PDFs are sampled increases.
