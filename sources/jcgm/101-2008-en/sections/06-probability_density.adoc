...
== Probability density functions for the input quantities (((probability density function (PDF),for input quantities)))

=== General

==== {blank} (((Bayes' theorem)))

This clause gives guidance on the assignment, in some common circumstances, of PDFs to the input quantities stem:[ii(X)_i] in the formulation stage of uncertainty evaluation. Such an assignment can be based on Bayes' theorem <<Gelman2004>> or the ((principle of maximum entropy)) <<Cox-Harris2006>>, <<Jaynes1989>>, <<Weise1992>>, <<Woger1987>>.
...
==== {blank} (((probability density function (PDF),joint)))
...
==== {blank} ((("indications, analysis of a series of")))(((probability density function (PDF),for input quantities)))(((Type A and Type B evaluations of uncertainty)))

When the stem:[ii(X)_i] are independent, PDFs stem:[g_{ii(X)_i} (xi_i)] are assigned individually based on an analysis of a series of indications (Type A evaluation of uncertainty) or based on scientific judgement using information <<Taylor1994>> such as historical data, calibrations, and expert judgement (Type B evaluation of uncertainty) <<JCGM-100,clause=3.3.5>>.


==== {blank} (((probability density function (PDF),joint)))

When some of the stem:[ii(X)_i] are mutually independent, PDFs are assigned individually to them and a joint PDF to the remainder.
...
==== {blank} (((probability density function (PDF),for input quantities)))

Information relevant to the assignment of PDFs to the stem:[ii(X)_i] is contained in the GUM <<JCGM-100,clause=4.3>>.


==== {blank} (((knowledge,"about an input quantity")))

Comprehensive guidance on the assignment of PDFs individually or jointly to the stem:[ii(X)_i] is beyond the scope of this Supplement. Such assigned PDFs encode the knowledge and expertise of the metrologist who formulates the model and who is ultimately responsible for the quality of the final results.
...
=== Bayes' theorem (((Bayes' theorem))) index-range:pdf_quantities[(((probability density function (PDF),for input quantities)))]

==== {blank} ((("indications, analysis of a series of")))(((random variable)))
...
==== {blank} ((("indications, analysis of a series of")))
...
==== {blank} (((knowledge,"about a quantity, incomplete")))
...
[[scls_6-4]]
=== Probability density function assignment for some common circumstances [[pdf_quantities]]
...
[[scls_6-4-2]]
==== Rectangular distributions (((rectangular distribution)))

[[scls_6-4-2-1]]
===== {blank} (((rectangular distribution,assignment to a quantity)))
...
===== {blank} (((rectangular distribution,probability density function)))
...
===== {blank} (((rectangular distribution,expectation and variance)))
...
[[scls_6-4-3]]
==== Rectangular distributions with inexactly prescribed limits (((rectangular distribution)))(((rectangular distribution with inexactly prescribed limits)))

[[scls_6-4-3-1]]
===== {blank} (((rectangular distribution with inexactly prescribed limits,assignment to a quantity)))

A quantity stem:[ii(X)] is known to lie between limits stem:[ii(A)] and stem:[ii(B)] with stem:[ii(A) < ii(B)], where the midpoint stem:[(ii(A) + ii(B))//2] of the interval defined by these limits is fixed and the length stem:[ii(B) - ii(A)] of the interval is not known exactly. stem:[ii(A)] is known to lie in the interval stem:[a +- d] and stem:[ii(B)] in stem:[b +- d], where stem:[a], stem:[b] and stem:[d], with stem:[d > 0] and stem:[a + d < b - d], are specified. If no other information is available concerning stem:[ii(X)], stem:[ii(A)] and stem:[ii(B)], the ((principle of maximum entropy)) can be applied to assign to stem:[ii(X)] a "curvilinear trapezoid" (a rectangular distribution with inexactly prescribed limits).


===== {blank} (((rectangular distribution with inexactly prescribed limits,probability density function)))
...
| Sum of two quantities assigned rectangular distributions(((rectangular distribution))) with lower and upper limits stem:[a_1], stem:[b_1] and stem:[a_2], stem:[b_2]
a| Trapezoidal: +
stem:["Trap"(a, b, ii(beta))] with stem:[a = a_1 + a_2], +
stem:[b = b_1 + b_2], +
stem:[ii(beta) = \|(b_1 - a_1) - (b_2 - a_2)\|//(b - a)]
...
stem:[t_{n-1}(bar(x),s^2//n)] with stem:[hat(x) = sum_{i=1}^n x_i//n], +
stem:[s^2 = sum_{i=1}^n (x_i hat(x))^2//(n-1)]
a| image::figure-table1-7.png[]
| <<scls_6-4-9-2>>

| Best estimate stem:[x], expanded uncertainty stem:[ii(U)_{"p"}], coverage factor stem:[k_{"p"}] and effective ((degrees of freedom)) stem:[nu_{"eff"}]
a| Scaled and shifted stem:[t]: +
stem:[t_{nu_{"eff"}}(x,(ii(U)_{"p"}//k_{"p"})^2)]
...
[[scls_6-4-3-3]]
===== {blank} (((rectangular distribution with inexactly prescribed limits,expectation and variance)))
...
[[scls_6-4-3-4]]
===== {blank} (((calibration,certificate)))(((rectangular distribution with inexactly prescribed limits,sampling from)))
...
[example]
====
A certificate states that a voltage stem:[ii(X)] lies in the interval stem:[10.0 "unitsml(V)" +- 0.1 "unitsml(V)"]. No other information is available concerning stem:[ii(X)], except that it is believed that the magnitude of the interval endpoints is the result of rounding correctly some numerical value (see <<scls_3-20>>). On this basis, that numerical value lies between stem:[0.05 "unitsml(V)"] and stem:[0.15 "unitsml(V)"], since the numerical value of every point in the interval (0.05, 0.15) rounded to one significant decimal digit is 0.1. The location of the interval can therefore be regarded as fixed, whereas its width is inexact. The best estimate of stem:[ii(X)] is stem:[x = 10.0 "unitsml(V)"] and, using <<eq4>> based on stem:[a = 9.9 "unitsml(V)"], stem:[b = 10.1 "unitsml(V)"] and stem:[d = 0.05 "unitsml(V)"], the associated standard uncertainty stem:[u(x)] is given by
...
Hence stem:[u(x) = (0.0036)^{1//2} = 0.060 "unitsml(V)"], which can be compared with stem:[0.2//sqrt(12) = 0.058 "unitsml(V)"] in the case of exact limits, given by replacing stem:[d] by zero. The use of exact limits in this case gives a numerical value for stem:[u(x)] that is stem:[4 %] smaller than that for inexact limits. The relevance of such a difference needs to be considered in the context of the application.
====


[[scls_6-4-4]]
==== Trapezoidal distributions (((trapezoidal distribution)))

===== {blank} (((trapezoidal distribution,assignment to a quantity)))

The assignment of a symmetric trapezoidal distribution to a quantity is discussed in the GUM <<JCGM-100,clause=4.3.9>>. Suppose a quantity stem:[ii(X)] is defined as the sum of two independent quantities stem:[ii(X)_1] and stem:[ii(X)_2]. Suppose, for stem:[i = 1] and stem:[i = 2], stem:[ii(X)_i] is assigned a ((rectangular distribution)) stem:[R(a_i, b_i)] with lower limit stem:[a_i] and upper limit stem:[b_i]. Then the distribution for stem:[ii(X)] is a symmetric trapezoidal distribution stem:["Trap"(a, b, ii(beta))] with lower limit stem:[a], upper limit stem:[b], and stem:[a] parameter stem:[ii(beta)] equal to the ratio of the semi-width of the top of the trapezoid to that of the base. The parameters of this trapezoidal distribution are related to those of the rectangular distributions by

[[eq5]]
[stem]
++++
a = a_1 + a_2, " " b = b_1 + b_2, " " " "ii(beta) = lambda_1/lambda_2,
++++
...
[[scls_6-4-4-2]]
===== {blank} (((convolution)))(((trapezoidal distribution,probability density function)))
...
===== {blank} (((trapezoidal distribution,expectation and variance)))

stem:[ii(X)] has expectation and variance 

[stem%unnumbered]
++++
ii(E)(ii(X)) = {a + b}/2, " " ii(V)(ii(X)) = {(b - a)^2}/24 (1 + ii(beta)^2).
++++


===== {blank} (((trapezoidal distribution,sampling from)))

To sample from stem:["Trap"(a, b, ii(beta))], make two draws stem:[r_1] and stem:[r_2] independently from the standard ((rectangular distribution)) stem:[R(0, 1)] (see <<scls_C-3-3>>), and form

[stem%unnumbered]
++++
xi = a + {b - a}/2 [(1 + ii(beta))r_1 + (1 + ii(beta))r_2].
++++


[[scls_6-4-5]]
==== Triangular distributions (((triangular distribution)))

===== {blank} (((triangular distribution,assignment to a quantity)))

Suppose a quantity stem:[ii(X)] is defined as the sum of two independent quantities, each assigned a ((rectangular distribution)) (see <<scls_6-4-4>>), but with equal semi-widths, i.e. stem:[b_1 - a_1 = b_2 - a_2]. It follows from <<eq5>> and <<eq6>> that stem:[lambda_1 = 0] and stem:[ii(beta) = 0]. The distribution for stem:[ii(X)] is the trapezoidal distribution stem:["Trap"(a, b, 0)], which reduces to the (symmetric) triangular distribution stem:[T(a, b)] over the interval stem:[[a,b\]].


===== {blank} (((triangular distribution,probability density function)))
...
===== {blank} (((triangular distribution,expectation and variance)))
...
===== {blank} (((triangular distribution,sampling from)))

To sample from stem:[T(a, b)], make two draws stem:[r_1] and stem:[r_2] independently from the standard ((rectangular distribution)) stem:[R(0, 1)] (see <<scls_C-3-3>>), and form
...
[[scls_6-4-6]]
==== Arc sine (U-shaped) distributions (((arc sine distribution)))

===== {blank} (((arc sine distribution,assignment to a quantity)))
...
[stem%unnumbered]
++++
ii(X) = {a + b}/2 + {b - a}/2 sin ii(Phi),
++++

where stem:[ii(Phi)] has the ((rectangular distribution)) stem:[R(0,2pi)].

[[arc-sine-pdf]]
===== {blank} (((arc sine distribution,probability density function)))
...
stem:[ii(Z)] has expectation 1/2 and variance 1/8. The distribution (9) is termed the arc sine distribution, since the corresponding ((distribution function)) is

[stem%unnumbered]
++++
ii(G)_{ii(Z)} (z) = 1/pi arcsin (2z - 1) + 1/2.
++++
...
===== {blank} (((arc sine distribution,expectation and variance)))
...
===== {blank} (((arc sine distribution,sampling from)))
...
[[scls_6-4-7-1]]
===== {blank} (((best estimate,of an input quantity)))(((Gaussian distribution)))(((Gaussian distribution,assignment to a quantity)))
...
===== {blank} (((Gaussian distribution,probability density function)))
...
===== {blank} (((Gaussian distribution,expectation and variance)))
...
[[scls_6-4-7-4]]
===== {blank} (((Gaussian distribution,sampling from)))
...
[[scls_6-4-8]]
==== Multivariate Gaussian distributions (((multivariate Gaussian distribution)))

[[scls_6-4-8-1]]
===== {blank} (((best estimate,of a vector input quantity)))(((multivariate Gaussian distribution,assignment to a vector quantity)))(((uncertainty matrix)))
...
===== {blank} (((multivariate Gaussian distribution,probability density function)))(((probability density function (PDF),joint)))
...
===== {blank} (((covariance matrix)))(((multivariate Gaussian distribution,expectation and covariance matrix)))

stem:[bb(X)] has expectation and covariance matrix

[stem%unnumbered]
++++
ii(E)(bb(X)) = x, " " ii(V) (bb(X)) = bb(U_x).
++++


[[scls_6-4-8-4]]
===== {blank} (((Cholesky decomposition)))(((multivariate Gaussian distribution,sampling from)))(((probability density function (PDF),joint)))

To sample from stem:[N(bb(x),bb(U_x))], make stem:[ii(N)] draws stem:[z_i], stem:[i = 1,...,ii(N)], independently from the standard Gaussian distribution stem:[N(0, 1)] (see <<scls_C-4>>), and form
...
[NOTE]
====
The multivariate Gaussian PDF <<eq11>> reduces to the product of stem:[ii(N)] univariate Gaussian PDFs when there are no covariance effects. In that case
...
[stem%unnumbered]
++++
g_bb(X) (bb(xi)) = prod_{i=1}^N g_{ii(X)_i} (xi_i),
++++
...
[stem%unnumbered]
++++
g_{ii(X)_i} (xi_i) = 1/{sqrt(2 pi) (x_i)} exp(- {(xi - x_i)^2}/{2u^2(x_i)}).
++++

====

==== stem:[t]-distributions (((t-distribution)))
...
[[scls_6-4-9-2]]
===== {blank} (((average,of a set of indications)))(((Bayes' theorem)))((("indications, analysis of a series of")))(((t-distribution,assignment to a quantity)))(((variance,of a set of indications)))

Suppose that a series of stem:[n] indications stem:[x_1,..., x_n] is available, regarded as being obtained independently from a quantity with unknown expectation stem:[mu_0] and unknown variance stem:[ii(sigma)_0^2] having Gaussian distribution stem:["N"(mu_0,sigma_0^2)]. The desired input quantity stem:[ii(X)] is taken to be equal to stem:[mu_0]. Then, assigning a non-informative joint prior distribution to stem:[mu_0] and stem:[ii(sigma)_0^2], and using Bayes' theorem, the marginal PDF for stem:[ii(X)] is a scaled and shifted stem:[t]-distribution stem:[t_{nu}(bar(x), s^2//n)] with stem:[nu = n - 1] ((degrees of freedom)), where
...
===== {blank} (((gamma function)))(((t-distribution,probability density function)))
...
===== {blank} (((average,of a set of indications)))(((best estimate,of an input quantity)))(((degrees of freedom,effective)))((("indications, analysis of a series of")))(((standard uncertainty,reliability of)))(((t-distribution,expectation and variance)))(((Type A and Type B evaluations of uncertainty)))(((Welch-Satterthwaite formula)))
...
[[scls_6-4-9-5]]
===== {blank} (((t-distribution,sampling from)))
...
[[scls_6-4-9-6]]
===== {blank} (((degrees of freedom,of a pooled standard deviation)))((("indications, analysis of a series of")))(((standard deviation,pooled)))

If instead of a standard deviation stem:[s] calculated from a single series of indications, a pooled standard deviation stem:[s_p] with stem:[nu_p] ((degrees of freedom)) obtained from stem:[ii(Q)] such sets,
...
[stem%unnumbered]
++++
g_X (xi) = {Gamma((nu_{"p"} + 1)//2)}/{Gamma(nu_{"p"}//2)sqrt(nu_{"p"} pi)} times 1/{s_{"p"}//sqrt(n)}[1 + 1/{nu_{"p"}}({xi - bar(x)}/{s_{"p"}//sqrt(n)})^2]^{-(nu_{"p"} + 1)//2}
++++
...
[[scls_6-4-9-7]]
===== {blank} (((best estimate,from a calibration certificate)))(((calibration,certificate)))(((coverage factor)))(((degrees of freedom,effective)))(((expanded uncertainty)))(((t-distribution,assignment to a quantity)))

If the source of information about a quantity stem:[ii(X)] is a calibration certificate <<JCGM-100,clause=4.3.1>> in which a best estimate stem:[x], the expanded uncertainty stem:[ii(U)_p], the coverage factor stem:[k_p] and the effective ((degrees of freedom)) stem:[nu_{"eff"}] are stated, then a scaled and shifted stem:[t]-distribution stem:[t_{nu} (x, (ii(U)_p//k_p)^2)] with stem:[nu = nu_{"eff"}] ((degrees of freedom)) should be assigned to stem:[ii(X)].
...
===== {blank} (((best estimate,of a non-negative input quantity)))(((exponential distribution,assignment to a quantity)))
...
===== {blank} (((exponential distribution,probability density function)))
...
===== {blank} (((exponential distribution,expectation and variance)))
...
[[scls_6-4-6-4]]
===== {blank} (((exponential distribution,sampling from)))
...
[[scls_6-4-11]]
==== Gamma distributions (((gamma distribution)))(((Poisson distribution)))

===== {blank} (((Bayes' theorem)))(((gamma distribution,assignment to a quantity)))

Suppose the quantity stem:[ii(X)] is the average number of objects present in a sample of a fixed size (e.g. the average number of particles in an air sample taken from a clean room, or the average number of photons emitted by a source in a specified time interval). Suppose stem:[q] is the number of objects counted in a sample of the specified size, and the counted number is assumed to be a quantity with unknown expectation having a Poisson distribution. Then, according to Bayes' theorem, after assigning a constant prior distribution to the expectation, a gamma distribution stem:[G(q + 1, 1)] would be assigned to stem:[ii(X)].


===== {blank} (((gamma distribution,probability density function)))
...
===== {blank} (((gamma distribution,expectation and variance)))
...
[[scls_6-4-11-4]]
===== {blank} (((chi-squared distribution)))(((Erlang distribution)))(((exponential distribution)))(((gamma distribution,sampling from)))
...
NOTE: If the counting is performed over several samples (according to the same Poisson distribution), and stem:[q_i] is the number of objects counted in the __i__th sample, of size stem:[ii(S)_i], then the distribution for the average number of objects in a sample of size stem:[ii(S) = sum_i ii(S)_i] is stem:[G(alpha,ii(beta))] with stem:[alpha = 1 + sum_i q_i] and stem:[ii(beta) = 1]. <<eq14>> and <<eq15>> apply with stem:[q = sum_i q_i].
...
[[scls_6-5]]
=== Probability distributions from previous uncertainty calculations (((Monte Carlo method (MCM))))(((probability distribution,from previous uncertainty calculations)))
