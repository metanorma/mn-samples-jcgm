...
stem:[t_{n-1}(hat(x),s^2//n)] with stem:[hat(x) = sum_{i=1}^n x_i//n], +
...
[stem]
++++
g_X (xi) = {ii(Gamma) (n//2)}/{ii(Gamma)((n-1)//2)sqrt((n - 1)pi)} times 1/{s//sqrt(n)} (1 + 1/{n - 1} ({xi - bar(x)}/{s//sqrt(n)}))^{-n//2},
++++
...
[stem%unnumbered]
++++
ii(Gamma) (z) = int_{0}^{oo} t^{z - 1} e^{-t} " d"t, " " " " z > 0
++++
...
[stem%unnumbered]
++++
g_X (xi) = {ii(Gamma)((nu_{"p"} + 1)//2)}/{ii(Gamma)(nu_{"p"}//2)sqrt(nu_{"p"} pi)} times 1/{s_{"p"}//sqrt(n)}[1 + 1/{nu_{"p"}}({xi - bar(x)}/{s_{"p"}//sqrt(n)})^2]^{-(nu_{"p"} + 1)//2}
++++
