...
[[cls_7]]
== Implementation of a Monte Carlo method (((Monte Carlo method (MCM))))
...
[[scls_7-2]]
=== Number of Monte Carlo trials (((Monte Carlo method (MCM),number of trials)))

==== {blank} (((coverage interval,from Monte Carlo method)))(((coverage probability)))(((probability density function (PDF),for output quantity)))

A value of stem:[ii(M)], the number of Monte Carlo trials, i.e. the number of model evaluations to be made, needs to be selected. It can be chosen a _priori_, in which case there will be no direct control over the quality of the numerical results provided by MCM. The reason is that the number of trials needed to provide these results to a prescribed ((numerical tolerance)) will depend on the "shape" of the PDF for the output quantity and on the coverage probability required. Also, the calculations are stochastic in nature, being based on random sampling.

NOTE: A value of stem:[ii(M) = 10^6] can often be expected to deliver a stem:[95 %] coverage interval for the output quantity such that this length is correct to one or two ((significant decimal digits)).


==== {blank}

The choice of a value of stem:[ii(M)] that is large compared with stem:[1//(1-p)], e.g. stem:[ii(M)] at least stem:[10^4] times greater than stem:[1//(1-p)], should be made. It can then be expected that stem:[ii(G)] will provide a reasonable discrete representation of stem:[G_Y (ii(eta))] in the regions near the endpoints of a stem:[100p " %"] coverage interval for stem:[ii(Y)].
...
The model is evaluated for each of the stem:[ii(M)] draws from the PDFs for the N input quantities. Specifically, denote the stem:[ii(M)] draws by stem:[bb(x)_1,...,bb(x)_M], where the __r__th draw stem:[bb(x)_r] contains stem:[x_{1,r},..., x_{N,r}], with stem:[x_{i,r}] a draw from the PDF for stem:[ii(X)_i]. Then, the model values are
...
The necessary modifications are made to <<scls_7-4-1>> if the stem:[ii(X)_i] are not independent and hence a joint PDF is assigned to them.
...
[[scls_7-5]]
=== Discrete representation of the distribution function for the output quantity (((distribution function,for output quantity)))

[[scls_7-5-1]]
==== {blank} (((mode)))(((probability)))(((skewness)))(((sorting algorithm)))

A discrete representation stem:[bb(G)] of the distribution function stem:[ii(G)_Y (ii(eta))] for the output quantity stem:[ii(Y)] can be obtained as follows:

. [[item_7-5-1a]]sort the model values stem:[y_r], stem:[r = 1,..., ii(M)], provided by MCM into non-decreasing order. Denote the sorted model values by stem:[y_{(r)}], stem:[r = 1,..., ii(M)];

. [[item_7-5-1b]]if necessary, make minute numerical perturbations to any replicate model values stem:[y_{(r)}] in such a way that the resulting complete set of stem:[y_{(r)}], stem:[r = 1,..., ii(M)], form a strictly increasing sequence (cf. condition <<item_5-10-1b>> in <<scls_5-10-1>>);

. [[item_7-5-1c]]take stem:[bb(G)] as the set stem:[y_{(r)}], stem:[r = 1,..., ii(M)].


NOTE: With reference to step <<item_7-5-1a>>, a sorting algorithm taking a number of operations proportional to stem:[ii(M) ln ii(M)] should be used <<Scowen1965>>. A naive algorithm would take a time proportional to stem:[ii(M)^2], making the computation time unnecessarily long. See <<scls_7-8>>.
...
NOTE: With reference to step <<item_7-5-1c>>, a variety of information can be deduced from stem:[bb(G)]. In particular, information supplementary to the ((expectation)) and standard deviation can be provided, such as measures of skewness and kurtosis, and other statistics such as the mode and the median.
...
==== {blank} (((probability density function (PDF),for output quantity)))

The stem:[y_{(r)}] (or stem:[y_r]), when assembled into a histogram (with suitable cell widths) form a frequency distribution that, when normalized to have unit area, provides an approximation to the PDF stem:[g_Y (ii(eta))] for stem:[ii(Y)]. Calculations are not generally carried out in terms of this histogram, the resolution of which depends on the choice of cell widths, but in terms of G. The histogram can, however, be useful as an aid to understanding the nature of the PDF, e.g. the extent of its asymmetry. See, however, <<scls_7-8-3>> <<note1_7-8-3>> regarding the use of a large numerical value of stem:[ii(M)].


==== {blank}

A continuous approximation to stem:[ii(G)_Y (ii(eta))] is sometimes useful. <<annexD>> contains a means for obtaining such an approximation.


[[scls_7-6]]
=== Estimate of the output quantity and the associated standard uncertainty (((average,of sampled model values)))(((coverage interval,from Monte Carlo method)))(((Estimate of output quantity,from Monte Carlo method)))(((probability density function (PDF),for input quantities)))(((standard deviation,of sampled model values)))(((standard uncertainty,from Monte Carlo method)))
...
NOTE: In some special circumstances, such as when one of the input quantities has been assigned a PDF based on the stem:[t]-distribution with fewer than three ((degrees of freedom)), the ((expectation)) and standard deviation of stem:[ii(Y)], as described by the PDF stem:[g_Y (ii(eta))], might not exist. <<eq16>> and <<eq17>> might not then provide meaningful results. A coverage interval for stem:[ii(Y)] (see <<scls_7-7>>) can, however, be formed, since stem:[bb(G)] is meaningful and can be determined.

NOTE: stem:[tilde(y)] will not in general agree with the model evaluated at the best estimates of the input quantities, since, for a non-linear model stem:[f(bb(X))], stem:[ii(E)(ii(Y)) = ii(E)(f(bb(X))) != f(ii(E)(bb(X)))] (cf. <<JCGM-100,clause=4.1.4>>). Irrespective of whether stem:[f] is linear or non-linear, in the limit as stem:[ii(M)] tends to infinity, stem:[tilde(y)] approaches stem:[ii(E)(f(bb(X)))] when stem:[ii(E)(f(bb(X)))] exists.


[[scls_7-7]]
=== Coverage interval for the output quantity (((coverage interval,from Monte Carlo method)))

==== {blank}

A coverage interval for stem:[ii(Y)] can be determined from the discrete representation stem:[bb(G)] of stem:[G_Y (ii(eta))] in an analogous manner to that in <<scls_5-3-2>> given stem:[ii(G)_Y (ii(eta))].


==== {blank} (((coverage probability)))

Let stem:[q = p ii(M)], if stem:[p ii(M)] is an integer. Otherwise, take stem:[q] to be the integer part of stem:[pii(M) + 1//2]. Then stem:[[y_{"low"},y_{"high"}\]] is a stem:[100p " %"] coverage interval for stem:[ii(Y)], where, for any stem:[r = 1,..., ii(M) - q], stem:[y_{"low"} = y_{(r)}] and stem:[y_{"high"} = y_{(r+q)}]. The probabilistically symmetric stem:[100p " %"] coverage interval is given by taking stem:[r = (ii(M) - q)//2], if stem:[(ii(M) - q)//2] is an integer, or the integer part of stem:[(ii(M) - q + 1)//2], otherwise. The shortest stem:[100p " %"] coverage interval is given by determining stem:[r^∗] such that, for stem:[r = 1,..., ii(M) - q, y_{(r^∗ + q)} - y_{(r^{"*"})} <= y_{(r+q)} - y_{(r)}].

NOTE: Because of the randomness in MCM, some of these stem:[ii(M) - q] interval lengths will be shorter than they would be on average, and some longer. So, by choosing the least such length, (the approximation to) the shortest stem:[100p " %"] coverage interval tends to be marginally shorter than that which would have been calculated from stem:[ii(G)_Y (ii(eta))], with the consequence that the typical coverage probability is less than stem:[100p " %"]. For large stem:[ii(M)], this coverage probability is negligibly less than stem:[100p " %"].

[example]
stem:[10^5] numbers were drawn from a pseudo-random number generator for the rectangular distribution in the interval stem:[[0,1\]], and the shortest stem:[95 %] coverage interval formed as above. This exercise was carried out stem:[1000] times. The average coverage probability was stem:[94.92 %] and the standard deviation of the stem:[1000] coverage probabilities stem:[0.06 %].


[[scls_7-8]]
=== Computation time (((Monte Carlo method (MCM),computation time)))

==== {blank} (((probability density function (PDF),for input quantities)))(((probability density function (PDF),joint)))

The computation time for MCM is dominated by that required for the following three steps:

. make stem:[ii(M)] draws from the PDF for each input quantity stem:[ii(X)_i] (or the joint PDF for stem:[bb(X)]);
...
If the model is simple and the input quantities are independent, the time in step <<item_7-8-1c>> can be expected to dominate, and the overall time taken is typically a few seconds for stem:[ii(M) = 10^6] on a personal computer operating at several GHz. Otherwise, let stem:[ii(T)_1] be the time taken to make one draw from the PDFs for the input quantities and stem:[T_2] that to make one evaluation of the model. Then, the overall time can be taken as essentially stem:[ii(M) xx (ii(T)_1 + ii(T)_2)], which, if the model is complicated, is dominated by the term stem:[ii(M) ii(T)_2].

[[note1_7-8-3]]
NOTE: If the model is simple and stem:[ii(M)] very large, e.g. stem:[10^8] or stem:[10^9], the sorting time may be excessive compared with the time taken to make the stem:[ii(M)] model evaluations. In such a case, calculations can instead be based on an approximation to stem:[g_Y (ii(eta))] derived from a suitable histogram of the stem:[y_r].
...
[stem%unnumbered]
++++
ii(Y) = cos ii(X)_1 + sin ii(X)_2 + tan^{-1} ii(X)_3 + exp(ii(X)_4) + ii(X)_5^(1//3).
++++

Assign a Gaussian PDF to each input quantity stem:[ii(X)_i]. Make stem:[ii(M) = 10^6] Monte Carlo trials. The relative computation times for (a) generating 5M random Gaussian numbers, (b) forming stem:[ii(M)] model values and (&#x200c;c) sorting the stem:[ii(M)] model values were respectively stem:[20 %], stem:[20 %] and stem:[60 %], with a total computation time of a few seconds on a personal computer operating at several GHz.
====


[[scls_7-9]]
=== Adaptive Monte Carlo procedure (((Monte Carlo method (MCM),adaptive procedure)))

[[scls_7-9-1]]
==== General (((Monte Carlo method (MCM),number of trials)))
...
[[ex1_7-9-2]]
[example]
The estimate of the output quantity for a nominally stem:[100 "unitsml(g)"] measurement standard of mass <<JCGM-100,clause=7.2.2>> is stem:[y = 100.02147 "unitsml(g)"]. The ((standard uncertainty)) stem:[u(y) = 0.00035"unitsml(g)"], both significant digits being regarded as meaningful. Thus, stem:[n_{"dig"} = 2] and stem:[u(y)] can be expressed as stem:[35 xx 10^{-5} "unitsml(g)"], and so stem:[c = 35] and stem:[cc(l) = -5]. Take stem:[delta = 1/2 xx 10^{-5}] stem:[g = 0.000005 "unitsml(g)"].

[example]
As <<ex1_7-9-2>> except that only one significant decimal digit in stem:[u(y)] is regarded as meaningful. Thus, stem:[n_{"dig"} = 1] and stem:[u(y) = 0.0004 "unitsml(g)" = 4 xx 10^{-4} "unitsml(g)"], giving stem:[c = 4] and stem:[cc(l) = -4]. Hence, stem:[delta = 1/2 xx 10^{-4}] stem:[g = 0.00005 "unitsml(g)"].

[example]
In a temperature measurement, stem:[u(y) = 2 ii(K)]. Then, stem:[n_{"dig"} = 1] and stem:[u(y) = 2 x 10^0 "unitsml(K)"], giving stem:[c = 2] and stem:[cc(l) = 0]. Thus, stem:[delta = 1/2 xx 10^0] stem:[K = 0.5 "unitsml(K)"].


==== Objective of adaptive procedure (((coverage probability)))
...
[[scls_7-9-4]]
==== Adaptive procedure (((coverage interval,probabilistically symmetric)))(((coverage interval,statistical)))(((estimate of output quantity,from Monte Carlo method)))(((level of confidence)))
...
where stem:[ii(J)] is the smallest integer greater than or equal to stem:[100//(1 - p)];
--
...
. [[item_7-9-4k]]if any of stem:[2s_y], stem:[2s_{u(y)}], stem:[2s_{y_{"low"}}] and stem:[2s_{y_{"high"}}] exceeds stem:[delta], increase stem:[h] by one and return to <<item_7-9-4d>>;
...
NOTE: The factor 2 used in step <<item_7-9-4k>> is based on regarding the averages as realizations of Gaussian variables, and corresponds to a coverage probability of approximately stem:[95 %].

[NOTE]
====
An alternative, non-adaptive approach for a stem:[95 %] probabilistically symmetric coverage interval, which can be obtained using the statistics of the binomial distribution <<David1981>>, is as follows. Select stem:[ii(M) = 10^5] or stem:[ii(M) = 10^6]. Form the interval stem:[[y_{(r)}, y_{(s)}\]], where, for stem:[ii(M) = 10^5], stem:[r = 2420] and stem:[s = 97581], or, for stem:[ii(M) = 10^6], stem:[r = 24747] and stem:[s = 975254]. This interval is a stem:[95 %] statistical coverage interval at the level of confidence 0.99 <<JCGM-100,clause=C.2.30>> <<Willink2004>>, i.e. the coverage probability will be no less than stem:[95 %] in at least stem:[99 %] of uses of MCM. The average coverage probability of such an interval will be stem:[(s - r)//(ii(M) + 1)], which is greater than stem:[95 %] by an amount that becomes smaller as stem:[ii(M)] is increased, viz. stem:[95.16 %] for stem:[ii(M) = 10^5] and stem:[95.05 %] for stem:[ii(M) = 10^6]. (There are other possibilities for stem:[r] and stem:[s]; they do not have to sum to stem:[ii(M) + 1]. A sufficient condition <<David1981,clause=2.6>> is that stem:[s - r] satisfies

[stem%unnumbered]
++++
sum_{j=s-r}^{ii(M)} " "^{ii(M)} ii(C)_j p^j (1 - p)^{ii(M)-j} < 1 - 0.99,
++++
