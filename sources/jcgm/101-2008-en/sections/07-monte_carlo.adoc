...
The choice of a value of stem:[ii(M)] that is large compared with stem:[1//(1-p)], e.g. stem:[ii(M)] at least stem:[10^4] times greater than stem:[1//(1-p)], should be made. It can then be expected that G will provide a reasonable discrete representation of stem:[G_Y (ii(eta))] in the regions near the endpoints of a stem:[100p " %"] coverage interval for stem:[ii(Y)].
...
Let stem:[q = pii(M)], if stem:[pii(M)] is an integer. Otherwise, take stem:[q] to be the integer part of stem:[pii(M) + 1//2]. Then stem:[[y_{"low"},y_{"high"}\]] is a stem:[100p " %"] coverage interval for stem:[ii(Y)], where, for any stem:[r = 1,..., ii(M) - q], stem:[y_{"low"} = y_{(r)}] and stem:[y_{"high"} = y_{(r+q)}]. The probabilistically symmetric stem:[100p " %"] coverage interval is given by taking stem:[r = (ii(M) - q)//2], if stem:[(ii(M) - q)//2] is an integer, or the integer part of stem:[(ii(M) - q + 1)//2], otherwise. The shortest stem:[100p " %"] coverage interval is given by determining stem:[r^∗] such that, for stem:[r = 1,..., ii(M) - q, y_{(r^∗ + q)} - y_{(r^*)} <= y_{(r+q)} - y_{(r)}].
