...
[NOTE]
====
An alternative, non-adaptive approach for a 95 % probabilistically symmetric coverage interval, which can be obtained using the statistics of the binomial distribution <<David1981>>, is as follows. Select stem:[ii(M) = 10^5] or stem:[ii(M) = 10^6]. Form the interval stem:[[y_{(r)}, y_{(s)}\]], where, for stem:[ii(M) = 10^5], stem:[r = 2420] and stem:[s = 97581], or, for stem:[ii(M) = 10^6], stem:[r = 24747] and stem:[s = 975254]. This interval is a 95 % statistical coverage interval at the level of confidence 0.99 <<JCGM-100,clause=C.2.30>> <<Willink2004>>, i.e. the coverage probability will be no less than 95 % in at least 99 % of uses of MCM. The average coverage probability of such an interval will be stem:[(s - r)//(ii(M) + 1)], which is greater than 95 % by an amount that becomes smaller as stem:[ii(M)] is increased, viz. 95.16 % for stem:[ii(M) = 10^5] and 95.05 % for stem:[ii(M) = 10^6]. (There are other possibilities for stem:[r] and stem:[s]; they do not have to sum to stem:[ii(M) + 1]. A sufficient condition <<David1981,clause=2.6>> is that stem:[s - r] satisfies
