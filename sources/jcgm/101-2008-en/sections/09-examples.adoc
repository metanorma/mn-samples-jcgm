...
==== {blank} (((rectangular distribution,sampling from)))
...
|===
| Method | stem:[ii(M)] | stem:[y] | stem:[u(y)] | Probabilistically symmetric stem:[95 %] coverage interval | stem:[d_{"low"}] | stem:[d_{"high"}] | GUF validated (stem:[delta = 0.05])?
...
<<fig7>> shows the length stem:[y_{"high"} - y_{"low"}] of the stem:[95 %] coverage interval for stem:[ii(Y)] (see <<scls_7-7>>), as a function of the probability at its left-hand endpoint, determined from stem:[bb(G)]. As expected for a symmetric PDF, the interval takes its shortest length when symmetrically located with respect to the ((expectation)).


[[fig7]]
.The length of the stem:[95 %] coverage interval, as a function of the probability at its left-hand endpoint, for the discrete representation stem:[ii(G)] of the distribution function obtained by applying MCM to the model <<eq21,(21)>> (<<scls_9-2-2-8>>, <<scls_9-4-2-2-11>>)
image::figure7.png[]
...
[[scls_9-2-3]]
==== Rectangularly distributed input quantities with the same width (((rectangular distribution)))
...
By following the analogous steps of <<scls_9-2-2-3>> to <<scls_9-2-2-5>>, the results in <<table3>> were obtained. The analytic solution for the endpoints of the probabilistically symmetric stem:[95 %] coverage interval, viz. stem:[+-2 sqrt(3)[2 - (3//5)^{1//4}\] ~~ +-3.88], was obtained as described in <<annexE>>.
...
The probabilistically symmetric stem:[95 %] coverage interval determined on the basis of the GUM un-certainty framework is in this case slightly more conservative than that obtained analytically. As for nor-mally distributed quantities, the validation procedure was applied (columns 6 to 8 of <<table3>>). As before, stem:[n_{"dig"} = 2], stem:[u(y) = 20 xx 10^{-1}], stem:[c = 20], stem:[cc(l) = -1] and stem:[delta = 0.05]. The endpoint differences stem:[d_{"low"}] and stem:[d_{"high"}] are larger than for the case of normally distributed quantities (<<table2>>). For the first of the two applications of the adaptive Monte Carlo procedure, the GUM uncertainty framework is validated. For the second application, it is not validated, although stem:[d_{"low"}] and stem:[d_{"high"}] for this application are close to the ((numerical tolerance)) stem:[delta = 0.05] (seen if more decimal digits than in <<table3>> are considered). Different validation results such as these are an occasional consequence of the stochastic nature of the Monte Carlo method, especially in a case such as that here.
...
[cols="8*",options="header"]
|===
| Method | stem:[ii(M)] | stem:[y] | stem:[u(y)] | Probabilistically symmetric stem:[95 %] coverage interval | stem:[d_{"low"}] | stem:[d_{"high"}] | GUF validated (stem:[delta = 0.5])?
...
<<fig9>> also shows the endpoints of the probabilistically symmetric stem:[95 %] coverage interval for stem:[ii(Y)] obtained from these approximations. The inner pair of vertical lines indicates the endpoints of the probabilistically symmetric stem:[95 %] coverage interval determined by MCM. The outer pair results from the GUM uncertainty framework, with a coverage factor of k = 1.96.
...
The probabilistically symmetric stem:[95 %] coverage interval determined on the basis of the GUM uncertainty framework in this case is more conservative than that obtained using MCM. Again, the validation procedure was applied (columns 6 to 8 of <<table4>>). Now, stem:[n_{"dig"} = 2], stem:[u(y) = 1.0 xx 10^1 = 10 xx 10^0], stem:[c = 10], stem:[cc(l) = 0] and stem:[delta = 1//2 xx 10^0 = 0.5]. For the two applications of the adaptive Monte Carlo procedure, the GUM uncertainty framework is not validated. For a ((numerical tolerance)) of one significant decimal digit in stem:[u(y)], i.e. stem:[n_{"dig"} = 1], for which stem:[delta = 5], the validation status would be positive in both cases, the stem:[95 %] coverage intervals all being stem:[[-2 xx 10^1, 2 xx 10^1\]]. See <<scls_4-13>>.
...
It is usual to work in terms of conventional masses. The conventional mass stem:[m_{W,c}] of stem:[W] is the mass of a (hypothetical) weight of density stem:[ii(rho)_0 = 8000 "unitsml(kg/m)"^3] that balances stem:[W] in air at density stem:[ii(rho)_{a_0} = 1.2 "unitsml(kg/m)"^3]. Thus,
...
[[scls_9-3-1-4]]
===== {blank} (((best estimate,of an input quantity)))(((Gaussian distribution,assignment to a quantity)))(((rectangular distribution,assignment to a quantity)))
...
NOTE: The quantity stem:[ii(rho)_{a_0}] in the mass calibration model <<eq24,(24)>> is assigned the value stem:[1.2 "unitsml(kg/m)"^3] with no associated uncertainty.
...
| stem:[m_{R,c}] | stem:[N(ii(mu),ii(sigma)^2)] | stem:[100000000 "unitsml(mg)"] | stem:[0.050 "unitsml(mg)"] | |
| stem:[delta m_{R,c}] | stem:[N(ii(mu),ii(sigma)^2)] | stem:[1.234 "unitsml(mg)"] | stem:[0.020 "unitsml(mg)"] | |
| stem:[ii(rho)_a] | stem:[R(a,b)] | | | stem:[1.20 "unitsml(kg/m)"^3] | stem:[0.10 "unitsml(kg/m)"^2]
| stem:[ii(rho)_W] | stem:[R(a,b)] | | | stem:[8 xx 10^3 "unitsml(kg/m)"^3] | stem:[1 xx 10^3 "unitsml(kg/m)"^3]
| stem:[ii(rho)_R] | stem:[R(a,b)] | | | stem:[8.00 xx 10^3 "unitsml(kg/m)"^3] | stem:[0.05 xx 10^3 "unitsml(kg/m)"^3]
|===
...
<<fig10>> shows the approximations to the PDF for stem:[delta m] obtained from the GUM uncertainty framework with first-order terms and MCM. The continuous curve represents a Gaussian PDF with parameters given by the GUM uncertainty framework. The inner pair of (broken) vertical lines indicates the shortest stem:[95 %] coverage interval for stem:[delta m] based on this PDF. The histogram is the scaled frequency distribution obtained using MCM as an approximation to the PDF. The outer pair of (continuous) vertical lines indicates the shortest stem:[95 %] coverage interval for stem:[delta m] based on the discrete representation of the distribution function determined as in <<scls_7-5>>.
...
|===
| Method | stem:[hat(delta m)//"unitsml(mg)"] | stem:[u(hat(delta m))] | Shortest stem:[95 %] coverage interval stem:[//"unitsml(mg)"] | stem:[d_{"low"}//"unitsml(mg)"] | stem:[d_{"high"}//"unitsml(mg)"] | GUF validated (stem:[delta = 0.005])?
...
The results show that, although the GUM uncertainty framework (first order) and MCM give estimates of stem:[delta m] in good agreement, the numerical values for the associated ((standard uncertainty)) are noticeably different. The value (stem:[0.0754 "unitsml(mg)"]) of stem:[u(hat(delta m))] returned by MCM is stem:[40 %] larger than that (stem:[0.0539 "unitsml(mg)"]) returned by the GUM uncertainty framework (first order). The latter is thus optimistic in this respect. There is good agreement between stem:[u(hat(delta m))] determined by MCM and that (stem:[0.0750 "unitsml(mg)"]) provided by the GUM uncertainty framework with higher-order terms.
...
===== {blank} (((covariance matrix)))(((probability density function (PDF),joint)))(((Gaussian distribution)))
...
stem:[delta y] and stem:[u(delta y)] can generally be formed analytically as the ((expectation)) and standard deviation of stem:[delta ii(Y)], as characterized by the PDF for stem:[delta ii(Y)]. See <<scls_F-1>>. The PDF for stem:[delta ii(Y)] can be formed analytically when stem:[x_1 = 0] and, in particular, used to determine the endpoints of the shortest stem:[95 %] coverage interval in that case. See <<scls_F-2>>.
...
|===
| stem:[x_1] 3+| Estimate stem:[delta y//10^{-6}] 4+| Standard uncertainty stem:[u(delta y)//10^{-6}] 4+| Shortest stem:[95 %] coverage interval for stem:[delta ii(Y) //10^{-6}]
...
<<fig11>> also shows the shortest stem:[95 %] coverage intervals for the corresponding approximations to the distribution function for stem:[delta ii(Y)]. The stem:[95 %] coverage interval, indicated by dotted vertical lines, as provided by the GUM uncertainty framework is infeasible: it is symmetric about stem:[delta ii(Y) = 0] and therefore erroneously implies there is a stem:[50 %] probability that stem:[delta ii(Y)] is negative. The continuous vertical lines are the endpoints of the shortest stem:[95 %] coverage interval derived from the analytic solution, as described in <<scls_F-2>>. The endpoints of the shortest stem:[95 %] coverage interval determined using MCM are indistinguishable to graphical accuracy from those for the analytic solution.
...
<<fig12>> shows the length of the stem:[95 %] coverage interval (see <<scls_7-7>>), as a function of the probability value at its left-hand endpoint, for the approximation to the PDF provided by MCM shown in <<fig11>>. The stem:[95 %] coverage interval does not take its shortest length when symmetrically located with respect to the ((expectation)) in this case. Indeed, the shortest stem:[95 %] coverage interval is as far-removed as possible from a probabilistically symmetric coverage interval, the left and right tail probabilities being stem:[0 %] and stem:[5 %], respectively, as opposed to stem:[2.5 %] and stem:[2.5 %]. This figure can be compared with that (<<fig7>>) for the additive model of <<scls_9-2>>, for which the PDF for stem:[ii(Y)] is symmetric about its ((expectation)).
...
<<fig13>> also shows the endpoints of the shortest stem:[95 %] coverage intervals obtained by the three approaches. The continuous vertical lines denote the endpoints of the interval provided by MCM, the broken vertical lines those resulting from the GUM uncertainty framework with first-order terms, and the dotted vertical lines from the GUM uncertainty framework with higher-order terms. The intervals provided by the GUM uncertainty framework are shifted to the left compared with the shortest stem:[95 %] coverage interval for MCM. As a consequence, they again


[[fig12]]
.The length of the stem:[95 %] coverage interval, as a function of the probability value at its left-hand endpoint, for the approximation to the distribution function obtained by applying MCM to the model <<eq28,(28)>> (<<scls_9-4-2-2-11>>)
image::figure12.png[]
...
include infeasible values of stem:[delta ii(Y)]. The shift is about stem:[70 %] of the ((standard uncertainty)). The interval provided by MCM has its left-hand endpoint at zero, the smallest feasible value.
...
<<table9>> contains the results obtained. Those from MCM indicate that although stem:[delta y] is unaffected by the correlation between the stem:[ii(X)_i], stem:[u(delta y)] is so influenced, more so for small stem:[x_1]. The stem:[95 %] coverage intervals are influenced accordingly.
...
|===
h| stem:[x_1] 3+h| Estimate stem:[delta y // 10^{-6}] 3+h| Standard uncertainty stem:[u(delta y)//10^{-6}] 3+h| Shortest stem:[95 %] coverage interval for stem:[delta ii(Y)//10^{-6}]
...
<<fig15>> and <<fig16>> show the PDFs provided by the GUM uncertainty framework with first-order terms (bell-shaped curves) and MCM (scaled frequency distributions) in the cases stem:[x_1 = 0.010] and stem:[x_1 = 0.050], respectively. The endpoints of the shortest stem:[95 %] coverage interval provided by the two approaches are also shown, as broken vertical lines for the GUM uncertainty framework and continuous vertical lines for MCM.

NOTE: Strictly, the conditions under which stem:[delta ii(Y)] can be characterized by a Gaussian PDF do not hold following an application of the GUM uncertainty framework in this circumstance (see <<scls_5-8>>) <<JCGM-100,clause=G.6.6>>. However, this PDF and the endpoints of the corresponding stem:[95 %] coverage interval are shown because such a characterization is commonly used.
...
The length of a nominally stem:[50 "unitsml(mm)"] gauge block is determined by comparing it with a known reference standard of the same nominal length. The direct output of the comparison of the two gauge blocks is the difference stem:[d] in their lengths given by
...
where stem:[ii(L)] is the length at stem:[20 "unitsml(degC)"] of the gauge block being calibrated, stem:[ii(L)_s] is the length of the reference standard at stem:[20 "unitsml(degC)"] as given in its calibration certificate, stem:[alpha] and stem:[alpha_s] are the coefficients of thermal expansion, respectively, of the gauge being calibrated and the reference standard, and stem:[ii(theta)] and stem:[ii(theta)_s] are the deviations in temperature from the stem:[20 "unitsml(degC)"] reference temperature, respectively, of the gauge block being calibrated and the reference standard.
...
The quantity stem:[ii(theta)], representing deviation of the temperature from stem:[20 "unitsml(degC)"] of the gauge block being calibrated, can be expressed as
...
where stem:[ii(theta)_0] is a quantity representing the average temperature deviation of the gauge block from stem:[20 "unitsml(degC)"] and a quantity describing a cyclic variation of the temperature deviation from stem:[ii(theta)_0].
...
| stem:[ii(L)_s] | stem:[t_{nu} (ii(mu), ii(sigma)^2)] | stem:[50000623 "unitsml(nm)"] | stem:[25 "unitsml(nm)"] | stem:[18] | | | 
| stem:[ii(D)] | stem:[t_{nu} (ii(mu), ii(sigma)^2)] | stem:[215 "unitsml(nm)"] | stem:[6 "unitsml(nm)"] | stem:[24] | | | 
| stem:[d_1] | stem:[t_{nu} (ii(mu), ii(sigma)^2)] | stem:[0 "unitsml(nm)"] | stem:[4 "unitsml(nm)"] | stem:[5]  | | |
| stem:[d_2] | stem:[t_{nu} (ii(mu), ii(sigma)^2)] | stem:[0 "unitsml(nm)"] | stem:[7 "unitsml(nm)"] | stem:[8]  | | |
| stem:[alpha_s] | stem:[R(a,b)] | | | | stem:[9.5 xx 10^{-6} "unitsml(degC^(-1))"] | stem:[13.5 xx 10^{-6} "unitsml(degC^(-1))"] |
| stem:[ii(theta)_0] | stem:[N(ii(mu),ii(sigma)^2)] | stem:[-0.1 "unitsml(degC)"] | stem:[0.2 "unitsml(degC)"] | | | |
| stem:[ii(Delta)] | stem:[U(a,b)] | | | | stem:[-0.5 "unitsml(degC)"] | stem:[0.5 "unitsml(degC)"] |
| stem:[delta alpha] | stem:["CTrap"(a,b,d)] | | | | stem:[-1.0 xx 10^{-6} "unitsml(degC^(-1))"] | stem:[1.0 xx 10^{-6} "unitsml(degC^(-1))"] | stem:[1.0 xx 10^{-6} "unitsml(degC^(-1))"]
| stem:[delta ii(theta)] | stem:["CTrap"(a,b,d)] | | | | stem:[-0.050 "unitsml(degC)"] | stem:[0.050 "unitsml(degC)"] | stem:[0.025 "unitsml(degC)"]
|===
...
The average stem:[hat ii(D)] of the five indications of the difference in lengths between the gauge block being calibrated and the reference standard is stem:[215 "unitsml(nm)"] <<JCGM-100,clause=H.1.5>>. The pooled experimental standard deviation characterizing the comparison of stem:[ii(L)] and stem:[ii(L)_s] was determined from 25 indications, obtained independently, of the difference in lengths of two standard gauge blocks, and equalled stem:[13 "unitsml(nm)"] <<JCGM-100,clause=H.1.3.2>>.
...
[stem%unnumbered]
++++
ii(mu) = 215 "unitsml(nm)", "    " ii(sigma) = 13/{sqrt(5)} "unitsml(nm)" = 6 "unitsml(nm)", "    " nu = 24.
++++
...
According to the calibration certificate of the comparator used to compare stem:[ii(L)] with stem:[ii(L)_s], the associated uncertainty due to random effects is stem:[0.01 "unitsml(um)"] for a coverage probability of stem:[95 %] and is obtained from six indications, obtained independently <<JCGM-100,clause=H.1.3.2>>.
...
[stem%unnumbered]
++++
ii(mu) = 0 "unitsml(nm)", " " ii(sigma) = {U_{0.95}} / {k_{0.95}} = 10/2.57 "unitsml(nm)" = 4 "unitsml(nm)"," " nu = 5.
++++
...
The uncertainty of the comparator due to systematic effects is given in the certificate as stem:[0.02 "unitsml(um)"] at the "three sigma level" <<JCGM-100,clause=H.1.3.2>>. This uncertainty may be assumed to be reliable to stem:[25 %], and thus the ((degrees of freedom)) is stem:[nu_{"eff"}(u(hat(d_2))) = 8] <<JCGM-100,clause=H.1.6>>.
...
[stem%unnumbered]
++++
ii(mu) = 0 "unitsml(nm)", " " ii(sigma) = {U_p}/{k_p} = 20/3 "unitsml(nm)" = 7 "unitsml(nm)", " " nu = 8.
++++
...
The coefficient of thermal expansion of the reference standard is given as stem:[hat{alpha_s} = 11.5 xx 10^{-6} "unitsml(degC^(-1))"] with possible values of this quantity represented by a rectangular distribution with limits stem:[+-2 xx 10^{-6} "unitsml(degC^(-1))"] <<JCGM-100,clause=H.1.3.3>>.


[[scls_9-5-2-6-2]]
====== Interpretation (((rectangular distribution,assignment to a quantity)))

Assign a rectangular distribution stem:[R(a, b)] (see <<scls_6-4-2>>) to stem:[alpha_s], with limits 

[stem%unnumbered]
++++
a = 9.5 xx 10^{-6} "unitsml(degC^(-1))","    " b = 13.5 xx 10^{-6} "unitsml(degC^(-1))". 
++++
...
The temperature of the test bed is reported as stem:[(19.9 +- 0.5) "unitsml(degC)"]. The average temperature deviation stem:[hat(ii(theta)_0) = -0.1 "unitsml(degC)"] is reported as having an associated ((standard uncertainty)) due to the uncertainty associated with the average temperature of the test bed of stem:[u(hat(ii(theta)_0)) = 0.2 "unitsml(degC)"] <<JCGM-100,clause=H.1.3.4>>.
...
[stem%unnumbered]
++++
ii(mu) = - 0.1 "unitsml(degC)", "    " ii(sigma) = 0.2 "unitsml(degC)"
++++
...
The temperature of the test bed is reported as stem:[(19.9 +- 0.5) "unitsml(degC)"]. The stated maximum offset of stem:[0.5 "unitsml(degC)"] for is said to represent the amplitude of an approximately cyclical variation of temperature under a thermostatic system. The cyclic variation of temperature results in a U-shaped (arc sine) distribution <<JCGM-100,clause=H.1.3.4>>.
...
[stem%unnumbered]
++++
a = -0.5 "unitsml(degC)", "    " b = 0.5 "unitsml(degC)".
++++
...
The estimated bounds on the variability of stem:[delta alpha] are stem:[+- 1 xx 10^{-6} "unitsml(degC^(-1))"], with an equal probability of stem:[delta alpha] having
any value within those bounds <<JCGM-100,clause=H.1.3.5>>. These bounds are deemed to be reliable to stem:[10 %], giving stem:[nu(u(hat(delta alpha)) = 50] <<JCGM-100,clause=H.1.6>>.


======  Interpretation (((rectangular distribution with inexactly prescribed limits,assignment to a quantity)))

Assign a rectangular distribution with inexactly prescribed limits (see <<scls_6-4-3>>) to stem:[delta alpha], with

[stem%unnumbered]
++++
a = -1.0 xx 10^{-6} "unitsml(degC^(-1))", "    " b = 1.0 xx 10^{-6} "unitsml(degC^(-1))", "    " d = 0.1 xx 10^{-6} "unitsml(degC^(-1))".
++++

The stated reliability of stem:[10 %] on the estimated bounds provides the basis for this value of stem:[d].
...
The reference standard and the gauge block being calibrated are expected to be at the same temperature, but the temperature difference stem:[delta ii(theta)] could lie with equal probability anywhere in the estimated interval stem:[-0.05 "unitsml(degC)"] to stem:[0.05 "unitsml(degC)"] <<JCGM-100,clause=H.1.3.6>>. This difference is believed to be reliable only to stem:[50 %], giving stem:[nu(u(hat(delta ii(theta)))) = 2] <<JCGM-100,clause=H.1.6>>.
...
[stem%unnumbered]
++++
a = -0.050 "unitsml(degC)", "    " b = 0.050 "unitsml(degC)", "    " d = 0.025 "unitsml(degC)".
++++

The stated reliability of stem:[50 %] provides the basis for this value of stem:[d].


==== Propagation and summarizing

===== The GUM uncertainty framework (((degrees of freedom,effective)))(((Welch-Satterthwaite formula)))
...
<<table11>> gives the results obtained for the approximate model <<eq37,(37)>> using the information summarized in <<table10>>. <<fig17>> shows the PDFs for stem:[delta ii(L)] obtained from the application of the GUM uncertainty framework (solid curve) and MCM (scaled frequency distribution). The distribution obtained from the GUM uncertainty framework is a stem:[t]-distribution with stem:[nu = 16] ((degrees of freedom)). The endpoints of the shortest stem:[99 %] coverage intervals for stem:[delta ii(L)] obtained from the PDFs, which are indicated as vertical lines, are visually indistinguishable.


===== {blank}

stem:[1.26 xx 10^6] trials were taken by the adaptive Monte Carlo procedure. The calculations were also carried out for a coverage probability of stem:[95 %], for which stem:[0.53 xx 10^6] trials were taken.
...
|===
| Method | stem:[hat(delta ii(L))//"unitsml(nm)"] | stem:[u(hat(delta ii(L)))//"unitsml(nm)"] | Shortest stem:[99 %] coverage interval for stem:[delta ii(L)//"unitsml(nm)"]
...
There are some modest differences in the results obtained. stem:[u(hat(delta ii(L)))] was stem:[4 "unitsml(nm)"] greater for the application of MCM than for the GUM uncertainty framework. The length of the stem:[99 %] coverage interval for stem:[delta ii(L)] was stem:[1 "unitsml(nm)"] greater.
