...
For the input estimate stem:[x_1 = 0], higher-order terms must be used when applying the law of propagation of uncertainty, because the partial derivatives of stem:[delta ii(Y)] with respect to stem:[ii(X)_1] and stem:[ii(X)_2], evaluated at stem:[ii(X)_1 = x_1 and ii(X)_2 = x_2], are identically zero when stem:[x_1 = x_2 = 0]. Thus, if the law of propagation of uncertainty with first-order terms only were applied, the resulting standard uncertainty would incorrectly be computed as zero.
...
[stem%unnumbered]
++++
mu = 215 " nm", " " ii(sigma) = 13/{sqrt(5)} " nm" = 6 nm, " " nu = 24.
++++
