...
It is usual to work in terms of conventional masses. The conventional mass stem:[m_{W,c}] of stem:[W] is the mass of a (hypothetical) weight of density stem:[rho_0 = 8000 " "rm(kg/m)^3] that balances stem:[W] in air at density stem:[rho_{a_0} = 1.2 " "rm(kg/m)^3]. Thus,
...
Consider the case where the standard and the signal generator are reflectionless, i.e. stem:[Gamma_{"S"} = Gamma_{"G"} = 0], and measured values are obtained of the real and imaginary parts stem:[ii(X)_1] and stem:[ii(X)_2] of stem:[Gamma_M = ii(X)_1 + j ii(X)_2], where stem:[j^2 = -1]. Since stem:[|Gamma_M|^2 = ii(X)_1^2 + ii(X)_2^2], formula <<eq25>> becomes
...
[stem]
++++
d = ii(L) (1 + alpha theta) - ii(L)_s(1 + alpha_s theta_s)
++++
...
[stem]
++++
ii(L) = {ii(L)_s (1 + alpha_ theta_s) + d} / {1 + alpha theta},
++++
...
[stem]
+++++
ii(L) = ii(L)_s + d + ii(L)_s(alpha_s theta_s - alpha theta).
+++++
...
[stem]
++++
ii(L) = {ii(L)_s [1 + alpha_s (theta - delta theta)] + d} / {1 + (alpha_s + delta alpha)theta}
++++
...
[stem]
++++
ii(L) = ii(L)_s + d - ii(L)_s(theta delta alpha + alpha_s delta theta).
++++
...
[stem]
++++
delta ii(L) = {ii(L)_s [1 + alpha_s (theta_0 + Delta - delta theta)] + ii(D) + d_1 + d_2} / {1 + (alpha_s + delta alpha)(theta_0 + Delta)}
++++
...
[stem]
++++
delta ii(L) = ii(L)_s + ii(D) + d_1 + d_2 - ii(L)_s[delta alpha (theta_0 + Delta) + alpha_s delta theta] - ii(L)_{"nom"}
++++
...
| stem:[L_s] | stem:[t_{nu} (ii(mu), ii(sigma)^2)] | stem:[50000623 " " rm(nm)] | 25 nm | 18 | | | 
| stem:[D] | stem:[t_{nu} (ii(mu), ii(sigma)^2)] | 215 nm | 6 nm | 24 | | | 
...
| stem:[theta_0] | stem:[N(ii(mu),ii(sigma)^2)] | stem:[-0.1 " °C"] | stem:[0.2 " °C"] | | | |
...
Assign a Gaussian distribution stem:[N(ii(mu),ii(sigma)^2)] (see <<scls_6-4-7>>) to stem:[theta_0], with
...
|===
| Method | stem:[hat(delta ii(L))//rm(nm)] | stem:[u(hat(delta ii(L)))//rm(nm)] | Shortest 99 % coverage interval for stem:[delta L//rm(nm)]
