...
NOTE: If stem:[bb(V)] is semi-positive definite, the eigendecomposition stem:[bb(V) = bb(Q wedge Q)^{sf(T)}], where stem:[bb(Q)] is an orthogonal matrix and stem:[^^] a diagonal matrix, can be formed. Then stem:[bb(^^)^{1//2} bb(Q)^{sf(T)}] can be used to obtain draws from stem:[N(bb(0), bb(V))], even if stem:[bb(V)] is rank deficient.
