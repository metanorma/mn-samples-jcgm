...
[[annexC]]
[appendix]
== Sampling from probability distributions (((probability density function (PDF),sampling from)))(((probability distribution,sampling from)))

=== General


==== {blank} (((Monte Carlo method (MCM))))
...
[[scls_C-2]]
=== General distributions (((rectangular distribution)))
...
. draw a random number stem:[ii(rho)] from the rectangular distribution stem:[R(0, 1)];
. [[item_c-2b]]determine stem:[xi] satisfying stem:[ii(G)_X (xi) = ii(rho)].

NOTE: The inversion required in step <<item_c-2b>>, that is forming stem:[xi = ii(G)_X^{-1} (ii(rho))], may be possible analytically. Otherwise it can be carried out numerically.

[example]
As an instance of analytical inversion, consider the exponential PDF for stem:[ii(X)], with stem:[ii(X)] having ((expectation)) stem:[x (> 0)], viz. stem:[g_X (xi) = exp(-xi//x)//x], for stem:[xi >= 0], and zero otherwise (see <<scls_6-4-10>>). Then, by integration, stem:[ii(G)_X (xi) = 1 - exp(-xi//x)], for stem:[xi >= 0], and zero otherwise. Hence stem:[xi = -x ln(1 - ii(rho))]. This result can be simplified slightly by using the fact that if a variable stem:[ii(Q)] has the rectangular distribution stem:[R(0, 1)], then so has stem:[1 - ii(Q)]. Hence, stem:[xi = -x ln ii(rho)].

NOTE: Numerically, stem:[xi] can generally be determined by solving the "zero-of-a-function" problem stem:[G_X (xi) - ii(rho) = 0]. Upper and lower bounds for stem:[xi] are typically easily found, in which case a recognized "bracketing" algorithm such as bisection or, more efficiently, a combination of linear interpolation and bisection <<Dekker1969>>, for example, can be used to determine stem:[xi].

NOTE: If the pseudo-random number generator for the rectangular distribution is to be used as a basis for generating numbers from another distribution, a draw of stem:[ii(rho)] equal to zero or one can cause failure of that generator. An example is the ((exponential distribution)) (see <<scls_6-4-10>>). Its PDF (<<arc-sine-pdf>>) is not defined for stem:[ii(rho)] equal to zero or one. The use of the generator given in <<scls_C-3-3>> would not give rise to a failure of that type.


[[scls_C-3]]
=== Rectangular distribution (((rectangular distribution,sampling from)))

==== General

===== {blank} (((rectangular number generator,quality of)))
...
[[scls_C-3-3]]
==== Procedure for generating pseudo-random numbers from a rectangular distribution (((rectangular number generator,recommended)))
...
[[scls_C-4]]
=== Gaussian distribution (((Gaussian distribution,sampling from)))

The procedure in <<tablec3>> provides draws from the standard ((Gaussian distribution)) stem:[N(0, 1)] using the Box-Muller transform <<Box1958>>. A draw from the ((Gaussian distribution)) stem:[N(ii(mu),ii(sigma)^2)] is given by stem:[ii(mu) + ii(sigma) z], where stem:[z] is a draw from stem:[N(0, 1)].
...
[[scls_C-5]]
=== Multivariate Gaussian distribution (((covariance matrix)))(((multivariate Gaussian distribution,sampling from)))

==== {blank}

The most important multivariate distribution is the multivariate (or joint) Gaussian distribution stem:[N(bb(mu),bb(V))], where stem:[bb(mu)] is a vector of expectations of dimension stem:[n xx 1] and stem:[ii(V)] a covariance matrix of dimension stem:[ii(N) xx ii(N)].


[[scls_C-5-2]]
==== {blank} (((Cholesky decomposition)))

Draws from stem:[N(bb(mu), bb(V))] <<Salter2000>>, <<Strang1997>> can be obtained using the procedure in <<tablec4>>.
...
NOTE: If stem:[bb(V)] is not positive definite, perhaps because of numerical rounding errors or other sources, stem:[bb(R)] may not exist. Moreover, in cases where one or more of the eigenvalues of stem:[bb(V)] is very small (but positive), the software implementation of the Cholesky factorization algorithm used may be unable to form stem:[bb(R)] because of the effects of floating-point errors. In either of these situations it is recommended that stem:[bb(V)] is "repaired", i.e. as small a change as possible is made to stem:[bb(V)] such that the Cholesky factor stem:[bb(R)] for the modified matrix is well defined. The resulting factor is exact for a covariance matrix that numerically is close to the original stem:[bb(V)]. A simple repair procedure is available <<Strang1997,page=322>> for this purpose, and is embodied in the MULTNORM generator <<Salter2000>>.


NOTE: If stem:[bb(V)] is semi-positive definite, the eigendecomposition stem:[bb(V) = bb(Q wedge Q)^{sf(T)}], where stem:[bb(Q)] is an orthogonal matrix and stem:[bb(^^)] a diagonal matrix, can be formed. Then stem:[bb(^^)^{1//2} bb(Q)^{sf(T)}] can be used to obtain draws from stem:[N(bb(0), bb(V))], even if stem:[bb(V)] is rank deficient.
...
[[scls_C-5-3]]
==== {blank} (((correlated input quantities)))
...
[[scls_C-6]]
=== stem:[t]-distribution (((t-distribution,sampling from)))
...
. If stem:[v < 1 - \|t\|//2] or stem:[v < (1 + t^2//nu)^{-(nu+1)//2}], accept stem:[t] as a draw from the stem:[t]-distribution; otherwise repeat from step <<item_C-5a>>
|===
