...
[stem%unnumbered]
++++
ii(V)(ii(X)) = ii(E)(ii(X)^2) - [ii(E)(ii(X))]^2.
++++
...
[stem%unnumbered]
++++
E(ii(X)^2) = [ii(E)(ii(X))]^2 + ii(V)(ii(X)) = x^2 + u^2(x),
++++

where stem:[x] is the best estimate of stem:[ii(X)] and stem:[u(x)] the ((standard uncertainty)) associated with stem:[x]. Thus, for the model <<eq28,(28)>>, viz. stem:[delta ii(Y) = 1 - ii(Y) = ii(X)_1^2 + ii(X)_2^2],
...
where, for stem:[i = 1] and stem:[i = 2], stem:[u^2(x_i^2) = ii(V) (ii(X)_i^2)], and stem:[u(x_1^2, x_2^2) = rm(Cov)(ii(X)_1^2, ii(X)_2^2)]. Then, applying Price's Theorem for Gaussian distributions <<Papoulis1965>>, <<Price1958>>
...
For the case stem:[x_1 = x_2 = r(x_1, x_2) = 0] and stem:[u(x_1) = u(x_2)], the PDF stem:[g_Y (ii(eta))] for stem:[ii(Y)] can be obtained analytically. It is valuable to have such a solution for further validation purposes. In the above circumstances,

[stem%unnumbered]
++++
delta ii(Y) = u^2 (x_1) [ii(X)_1^2/{u^2 (x_1)} + ii(X)_2^2/{u^2(x_2)}].
++++
...
The application of a general formula <<Rice1995,page=57-61>> for the PDF stem:[g_Y (ii(eta))] of a differentiable and strictly decreasing function of a variable (here stem:[ii(Z)]) with a specified PDF yields

[stem%unnumbered]
++++
g_Y (ii(eta)) = 1/{u^2 (x_1)} χ_2^2 (ii(eta)/{u^2(x_1)}) = 1/{2u^2 (x_1)} exp(- ii(eta)/{2u^2(x_1)}), " " ii(eta) >= 0.
++++
...
[stem%unnumbered]
++++
delta y = E(delta ii(Y)) = int_{0}^{oo} ii(eta) g_Y (ii(eta)) " d"ii(eta) = 2u^2 (x_1)
++++
...
[stem%unnumbered]
++++
u^2 (delta y) = V(delta ii(Y)) = int_{0}^{oo} (ii(eta) - y)^2 g_Y (ii(eta)) " d"ii(eta) = 4u^4 (x_1),
++++
...
[stem]
++++
G_Y (ii(eta)) = 1 - exp(- ii(eta)/{2u^2(x_1)}), " " ii(eta) >= 0.
++++


==== {blank} (((coverage interval)))

Let stem:[delta y_{alpha}] be that stem:[ii(eta)] in <<eqf3>> corresponding to stem:[ii(G)_Y (ii(eta)) = alpha] for any stem:[alpha] satisfying stem:[0 <= alpha <= 1-p]. Then
...
The shortest stem:[100p " %"] coverage interval is given by determining stem:[alpha] to minimize stem:[ii(H)(alpha)] (see <<scls_5-3-4>>). Since stem:[ii(H)(alpha)] is a strictly increasing function of stem:[alpha] for stem:[0 <= alpha <= 1 - p], stem:[ii(H)(alpha)] is minimized when stem:[alpha = 0]. Thus, the shortest stem:[100p " %"] coverage interval for stem:[delta ii(Y)] is
...
For stem:[u(x_1) = 0.005], the shortest stem:[95 %] coverage interval is
...
The stem:[95 %] probabilistically symmetric coverage interval for stem:[delta ii(Y)] is given by setting stem:[alpha = (1 - p)//2] (see <<scls_5-3-3>>):
...
which is stem:[20 %] longer than the shortest stem:[95 %] coverage interval.

NOTE: The above analysis is indicative of an analytical approach that can be applied to some problems of this type. In this particular case, the results could in fact have been obtained more directly, since stem:[g_Y(ii(eta))] is strictly increasing and the shortest coverage interval is always in the region of highest density.


[[scls_F-3]]
=== GUM uncertainty framework applied to the comparison loss problem (((GUM uncertainty framework (GUF))))

==== Uncorrelated input quantities

===== {blank} (((Gaussian distribution,assignment to a quantity)))

The comparison loss problem considered in <<scls_9-4>> has as the model of measurement

[stem%unnumbered]
++++
delta ii(Y) = f(bb(X)) = f(ii(X)_1, ii(X)_2) = ii(X)_1^2 + ii(X)_2^2,
++++
...
===== {blank} (((estimate of output quantity,from GUM uncertainty framework)))(((partial derivative)))
...
[stem%unnumbered]
++++
{partial f}/{partial ii(X)_i} = 2 ii(X)_i, "    " {partial^2 f}/{partial ii(X)_i^2} = 2.
++++
...
[stem%unnumbered]
++++
{:1/2 [{partial^2 f}/{partial ii(X)_1^2} + {partial^2 f}/{partial ii(X)_2^2}]|_{bb(X)=bb(x)} u^2 (x_1) u^2 (x_2)
++++
...
A stem:[95 %] coverage interval for stem:[delta ii(Y)] is given by
...
[[scls_F-3-2]]
==== Correlated input quantities (((correlated input quantities)))

===== {blank} (((uncertainty matrix)))
...
[stem]
++++
{:(u^2(delta y),= {:[({partial f}/{partial ii(X)_1})^2 u^2(x_1) + ({partial f}/{partial ii(X)_2})^2 u^2(x_2) + 2 {partial f}/{partial ii(X)_1} {partial f}/{partial ii(X)_2} r(x_1,x_2)u(x_1)u(x_2)]|_{bb(X)=bb(x)}),
